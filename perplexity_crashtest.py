    ...
    with open(args.output_file, "w") as f:
        ...
        for i, (token, perp) in enumerate(zip(tokens, perplexities)):
            ...
            if i * len(token) < response_start:
                f.write(f"Token {i:3d} | Perplexity: {perp:6.2f} | Token: '{visible_token}'\n")
        ...
        prompt_perp = sum(perplexities[:response_start]) / response_start if response_start > 0 else 0
        response_perp = sum(perplexities[response_start:]) / (len(perplexities) - response_start)
